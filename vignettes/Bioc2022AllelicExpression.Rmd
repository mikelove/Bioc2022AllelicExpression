...
Here we switch to using TSS-level data. We first filter out
non-informative features as shown in the previous step:
...
filter out rows with insufficient counts and calculate the
statistics. Here, we skip scaling since we are comparing alleles
within samples. By default, `labelKeep` label features with a minimal
count of 10 in at least 3 samples.

Here we are performing two types of allelic differentiation
tests. *global AI test* tests allelic imbalance across all
samples. *Dynamic AI test* can detecte non-zero correlation between
the log allelic fold change and a continuous variable (time). 
...
The results are stored in `mcols()`. We can examine how many TSS
groups are in a 5% FDR set by using: 
...
5% FDR set. One can specify the threshold by using `alpha = ...`
option.
...
`plotAllelicGene` function in `fishpond` can be used to visualize
Swish test results ($-log(qvalue)$ and log2FC), allelic proportions
and isoform proportions (calculated from TPM) with gene model. You can
also provide the argument with gene symbol `symbol = "Soat1"`. Either
*TxDb* or *EnsDb* can be use to plot gene models at the top.

```{r globalGene}
plotAllelicGene(global, gene = "ENSMUSG00000026600", db = edb, labels=list(a2="B6",a1="CAST"))
```

We can verify the `plotAllelicGene` with `plotInfReps`. We can see B6
has a higher count than CAST in majority of the samples. One can
customize the label of alleles in the function with the following
code, switching from `a2, a1` to `B6, CAST`. 
...
We can also use heatmap to visualize the difference in expression for
the Transcript groups, using `plotAllelicHeatmap`, using
`pheatmap`. Darker green in the annotation bar represents a more
significant *qvalue* in differential testing. Red is an indication of
high alternative allele ratio while blue is high reference allele
ratio. 
...
`plotInfReps` is a great way to visualize how allelic counts changes
with time. Using `lines` can help us better visualize the trend.
...
imbalance data. We first need to group the timepoints. In this
example, we are grouping Day 2-6, Day 8-12 and Day 14-18 together. How
the group the timepoints can be determined by your biological
motivation or the strategy to maxmize the difference in the ratio by
looking at `plotInfReps` plot. The plot shows allelic proportions with
grouped timepoints, as well as isoform proportion. We can see an
increasing in B6 expression with time for the first transcript group
and an increasing in CAST expression with time for the second
transcript group. 
...
We can add a time bar on the top of the heatmap to indicate time by
defining `col_dat`.
